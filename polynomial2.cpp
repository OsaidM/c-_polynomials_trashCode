...
void productPoly(int *firstArrayOfTerms[], int *secondArrayOfTerms[], int firstSize, int secondSize)
{
    ...
    if(firstSize > secondSize){ // if firstSize is bigger
        ...
        int** productOfPoly = new int*[firstSize];
        for(int i=0;i<firstSize; i++){
            productOfPoly[i] = new int[2];
        }
        ...
        {    
            for(int j = 0; j < secondSize; j++){
                if(firstArrayOfTerms[i][1] == secondArrayOfTerms[j][1]){
                    productOfPoly[i][0] = firstArrayOfTerms[i][0] * secondArrayOfTerms[j][0];
                    productOfPoly[i][1] = firstArrayOfTerms[i][1] + secondArrayOfTerms[j][1];
                    break;
                }else{
                    productOfPoly[i][0] = firstArrayOfTerms[i][0];
                    productOfPoly[i][1] = firstArrayOfTerms[i][1];
                }
                
            }
            // sum[i] = A[i];
        }
        printPoly(productOfPoly, firstSize);
    }else if(secondSize > firstSize){ // if secondSize size is bigger
        int** productOfPoly = new int*[secondSize];
        for(int i=0;i<secondSize; i++){
            productOfPoly[i] = new int[2];
        }
        ...
        {    
            for(int j = 0; j < firstSize; j++){
                if(firstArrayOfTerms[i][1] == secondArrayOfTerms[j][1]){
                    ...
                    productOfPoly[i][0] = secondArrayOfTerms[i][0];
                    productOfPoly[i][1] = secondArrayOfTerms[i][1];
                }
            }
            // sum[i] = A[i];
        }
        printPoly(productOfPoly, secondSize);
    }else{ // if the same size
        /* 
        * add cases to cover
        * first Poly: 3x^2+4x
        ...
        */
        ...
        int size = firstSize*2;
        int newSize = 0;
        int** productOfPoly = new int*[size];
        for(int i=0;i<size; i++){
            productOfPoly[i] = new int[2];
        }
        ...
        for (int i = 0; i < firstSize; i++)
        {
            if(newSize > size) break;
            if(firstArrayOfTerms[i][1] == secondArrayOfTerms[i][1]){
                productOfPoly[i][0] = firstArrayOfTerms[i][0] * secondArrayOfTerms[i][0];
                productOfPoly[i][1] = firstArrayOfTerms[i][1] + secondArrayOfTerms[i][1];
                newSize+=1;
            }else{
                productOfPoly[newSize][0] = firstArrayOfTerms[i][0];
                productOfPoly[newSize][1] = firstArrayOfTerms[i][1];
                productOfPoly[newSize+1][0] = secondArrayOfTerms[i][0];
                productOfPoly[newSize+1][1] = secondArrayOfTerms[i][1];
                newSize+=2;
                cout << endl <<newSize<< endl;
        ...
        for(int z =0; z < newSize; z++){
            cout << "[" << productOfPoly[z][0] << ", " << productOfPoly[z][1] << "]" << endl;
        }
        printPoly(productOfPoly, newSize);
    }
}
...
int main()
{
    ...
    if (userInput == 1) {
        ...
        cout << endl <<"Result of Producting the  Polynimoial... " << endl;
